...
class Recent
  ...
  def parse_results(html_content)
    ...
    movie_elements.map { |movie| extract_movie_data(movie) }.compact
  end
  ...
  def extract_movie_data(movie_element)
    ...
    image_url = image_url.sub(%r{^//}, 'https://') if image_url.start_with?('//')
    ...
    release_datetime = DateTime.new(release_year || 1970)
